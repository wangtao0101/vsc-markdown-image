...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.info(
    'Congratulations, your extension "markdown-sync-image" is now active!'
  );
    ...
    async () => {
      ...
      try {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          maxWidth.push(
            config.base.imageWidth < width ? config.base.imageWidth : 0
          );
          let name = config.base.fileNameFormat
            ? (await utils.formatName(
                config.base.fileNameFormat,
                images[i],
                savePath === images[i]
              )) + (images[i] ? path.extname(images[i]) : ".png")
            : images[i];
        ...
        for (let i = 0; i < urls.length; i++) {
          ...
          if (
            selections?.length === 1 &&
            editor?.document.getText(selections[0])
          ) {
            selection = `${editor?.document.getText(selections[0])} ${i + 1}`;
          } else if (
            selections?.[i] &&
            editor?.document.getText(selections[i])
          ) {
            selection =
              selections?.[i] && editor?.document.getText(selections[i]);
          }

          if (config.base.uploadMethod !== "Data URL") {
            if (config.base.urlEncode) {
              urls[i] = encodeURIComponent(urls[i].toString())
                .replace(/%5C/g, "\\")
                .replace(/%2F/g, "/")
                .replace(/%3A/g, ":");
            }
            let text = utils.formatCode(
              urls[i],
              selection,
              maxWidth[i],
              config.base.codeType
            );
        ...
        vscode.window.showErrorMessage(
          `${$l["something_wrong"]}${
            e.message in $l ? (<any>$l)[e.message] : e.message
          }\n${e.toString()}`
        );
      }
  ...
  let richTextCommand = vscode.commands.registerCommand(
    "markdown-sync-image.paste-rich-text",
    async () => {
      let stop = () => {};
      try {
        let editor = vscode.window.activeTextEditor;
  ...
  let copyMarkdownCommand = vscode.commands.registerCommand(
    "markdown-sync-image.paste-markdown",
    async () => {
      let stop = () => {};
